...
class Superwar {
  ...
  // Dustin and Lucus want to display the scores of the teams. So to display the scores, add logic to the calculateScore() method, use reduce array method to loop through players, such that it should return the score for the both the teams.
  // Fight
  fight = () => {
  ...
  calculateScore = () => {
    ...
    this.players.reduce((acc,player) => {
      if (player.type == 'hero') {
        player.win = setWinner[0];
        score[0] += player.wins;
      } else {
        player.win = setWinner[1];
        score[1] += player.wins;
      }
      return acc;
    },0);
  ...
  checkWin = () => {
    ...
    let result = 'endure';
    let health = {
      hero: this.totalStrength(0),
      villain: this.totalStrength(1),
    };
  ...
  totalStrength = (type) => {
    ...
    this.players.map((player) => {
      if (player.type == 'hero') {
        strength[0] += player.strength;
      } else {
        strength[1] += player.strength;
      }
    });
  ...
  announceWinner = (score) => {
    if (score[0] == score[1]) alert('Its a draw!');
    else if (score[0] > score[1]) alert('Heroes Win!');
